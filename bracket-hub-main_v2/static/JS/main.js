...
function adicionarJogadorLive(matchId) {
    ...
    if (nomes.length === 0) {
        exibirNotificacao('Adicione os jogadores primeiro à partida para colocar na live.', 'warning', 3000);
    } else {
...
function abrirModalPlacar(matchId) {
    ...
    if(!m || !m.p1 || !m.p2 || m.p1 === 'Bye' || m.p2 === 'Bye' || m.p1 === '' || m.p2 === '') {
        exibirNotificacao('Esta partida nao pode ser disputada (Bye ou aguardando).', 'warning', 3000);
        return;
    }
    
    if (m.status === 'completed') {
        exibirNotificacao('Esta partida ja foi concluida.', 'warning', 3000);
        return;
    }
...
function confirmarPlacarPartida() {
    ...
    if (!m) {
        exibirNotificacao('Partida nao encontrada.', 'danger', 3000);
        return;
    }
    ...
    if(s1 === s2) {
        exibirNotificacao('Partidas nao podem terminar em empate.', 'danger', 3000);
        return;
    }

    if (!m.p1 || !m.p2 || m.p1 === 'Bye' || m.p2 === 'Bye' || m.p1 === '' || m.p2 === '') {
        exibirNotificacao('Partida incompleta. Aguardando jogadores.', 'warning', 3000);
        return;
    }
    ...
    let partidasKeys = Object.keys(torneioAtivo.partidas).filter(function(id) {
        return id.startsWith('w-match-');
    });
    let totalPartidas = partidasKeys.length;
    let totalJogadores = totalPartidas + 1;
    
    let proximaPartidaNum = null;
    
    if (totalJogadores === 4) {
        if (numPartida === 1 || numPartida === 2) {
            proximaPartidaNum = 3;
        }
    }
...
function gerarChaveamentoDuplo() {
    ...
    if (totalJogadores < 2) {
        exibirNotificacao('Adicione pelo menos 2 jogadores para gerar uma chave.', 'warning', 4000);
        return;
    }
    
    if ((totalJogadores & (totalJogadores - 1)) !== 0) {
        exibirNotificacao('O numero de jogadores (' + totalJogadores + ') deve ser potencia de 2 (2, 4, 8, 16).', 'warning', 4000);
        return;
    }
    ...
    let jogadoresEmbaralhados = [...p].sort(function() { return Math.random() - 0.5; });
    
    let partidasPrimeiraFase = totalJogadores / 2;
    
    for (let i = 0; i < partidasPrimeiraFase; i++) {
        let id = 'w-match-' + (i + 1);
        let idx1 = i * 2;
        let idx2 = i * 2 + 1;
    ...
    const primeiraPartidaGerada = Object.keys(torneioAtivo.partidas).find(function(id) {
        const partida = torneioAtivo.partidas[id];
    ...
    exibirNotificacao('Chave gerada com ' + totalJogadores + ' jogadores!', 'success', 4000);
}
