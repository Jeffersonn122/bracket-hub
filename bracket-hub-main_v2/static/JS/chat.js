...
function adicionarMensagemPartida(matchId, author, text, type) {
    type = type || 'player';
    if (!chatsPorPartida[matchId]) {
        chatsPorPartida[matchId] = [];
    }
    chatsPorPartida[matchId].push({ author: author, text: text, type: type });
    renderizarPainelModerador();
    renderizarDrawerChat();
}
...
function renderizarDrawerChat() {
    ...
    body.innerHTML = mensagens.length
        ? mensagens.map(function(msg) {
            return '<div class="chat-drawer-message ' + msg.type + '"><strong>' + msg.author + '</strong><div>' + msg.text + '</div></div>';
        }).join('')
...
function renderizarPainelModerador() {
    ...
    const conversas = Object.entries(chatsPorPartida)
        .filter(function(entry) { return Array.isArray(entry[1]) && entry[1].length; })
        .slice(-4)
        .map(function(entry) {
            const matchId = entry[0];
            const mensagens = entry[1];
